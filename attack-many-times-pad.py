import binascii
import codecs
from copy import deepcopy


def decode(text: str):
    # TODO: hex decode logic here is not optimized, need to find a better way
    tmp = []
    new_string = ""
    tmp += binascii.a2b_hex(text)
    for t in tmp:
        new_string += chr(t)

    return new_string


def strxor(a, b):     # xor two strings of different lengths
    if len(a) > len(b):
       return "".join([chr(ord(x) ^ ord(y)) for (x, y) in zip(a[:len(b)], b)])
    else:
       return "".join([chr(ord(x) ^ ord(y)) for (x, y) in zip(a, b[:len(a)])])


if __name__ == "__main__":
    # Known cypher texts
    ciphers = [
        "",
        "234c02ecbbfbafa3ed18510abd11fa724fcda2018a1a8342cf064bbde548b12b07df44ba7191d9606ef4081ffde5ad46a5069d9f7f543bedb9c861bf29c7e205132eda9382b0bc2c5c4b45f919cf3a9f1cb74151f6d551f4480c82b2cb24cc5b028aa76eb7b4ab24171ab3cdadb8356f",
        "32510ba9a7b2bba9b8005d43a304b5714cc0bb0c8a34884dd91304b8ad40b62b07df44ba6e9d8a2368e51d04e0e7b207b70b9b8261112bacb6c866a232dfe257527dc29398f5f3251a0d47e503c66e935de81230b59b7afb5f41afa8d661cb",
        "",
        "3f561ba9adb4b6ebec54424ba317b564418fac0dd35f8c08d31a1fe9e24fe56808c213f17c81d9607cee021dafe1e001b21ade877a5e68bea88d61b93ac5ee0d562e8e9582f5ef375f0a4ae20ed86e935de81230b59b73fb4302cd95d770c65b40aaa065f2a5e33a5a0bb5dcaba43722130f042f8ec85b7c2070",
        "",
        "",
        "315c4eeaa8b5f8bffd11155ea506b56041c6a00c8a08854dd21a4bbde54ce56801d943ba708b8a3574f40c00fff9e00fa1439fd0654327a3bfc860b92f89ee04132ecb9298f5fd2d5e4b45e40ecc3b9d59e9417df7c95bba410e9aa2ca24c5474da2f276baa3ac325918b2daada43d6712150441c2e04f6565517f317da9d3",
        "",
        "466d06ece998b7a2fb1d464fed2ced7641ddaa3cc31c9941cf110abbf409ed39598005b3399ccfafb61d0315fca0a314be138a9f32503bedac8067f03adbf3575c3b8edc9ba7f537530541ab0f9f3cd04ff50d66f1d559ba520e89a2cb2a83"
    ]
    # Ciphertext we need to decrypt, also encrypted with the same key as the ciphertexts above
    target_cipher = "32510ba9babebbbefd001547a810e67149caee11d945cd7fc81a05e9f85aac650e9052ba6a8cd8257bf14d13e6f0a803b54fde9e77472dbff89d71b57bddef121336cb85ccb8f3315f4b52e301d16e9f52f904"
    ciphers.append(target_cipher)

    # Change every ciphertext from hex to ASCII
    ciphers = [decode(cipher) for cipher in ciphers]
    target = ciphers[-1]

    k = "The secret message is: When using a stream cipher, never use the key more than once"

    print("TARGET:")
    print(target + "\n")

    # Finding characters of target ciphertext
    target_plain = ""
    plains = ["" for plain in ciphers[:-1]]
    for idx, char in enumerate(target):
        while True:
            tmp_plains = deepcopy(plains)
            print("Current target char: " + str(hex(ord(char))))
            try_this_char = input("TRY THIS CHARACTER NEXT: ")
            for _idx, known_cipher in enumerate(ciphers[:-1]):
                target_xor_ct = chr((ord(char) ^ ord(known_cipher[idx])))
                target_xor_ct = chr(ord(target_xor_ct) ^ ord(try_this_char))
                tmp_plains[_idx] += target_xor_ct

            print("CURRENT STATE:")
            for _idx, tmp_plain in enumerate(tmp_plains):
                print("plain[" + str(_idx) + "]:")
                print(tmp_plain)
            print("")
            choose_ok = input("IS THIS STATE OK? y/n/h: ")

            if choose_ok not in ["y", "h"]:
                continue

            if choose_ok == "y":  # If "y", save the current state and print it out
                print("STATE AFTER ALTERING:")
                plains = deepcopy(tmp_plains)
                for _idx, plain in enumerate(plains):
                    print("plain[" + str(_idx) + "]:")
                    print(plain)
                target_plain += try_this_char
                print("target plaintext:" + target_plain)
                break

            if choose_ok == "h":  # If "h", help with finding the next char to choose
                print("Index to find = " + str(idx))
                choose_plain = int(input("choose a plaintext index in which you know the next character [0-9]: "))
                choose_next_character = input("which char do you think is next in choosen plaintext?")

                next_to_enter = chr((ord(char) ^ ord(ciphers[choose_plain][idx])))
                next_to_enter = chr(ord(next_to_enter) ^ ord(choose_next_character))
                print("then you need to enter: " + next_to_enter)

    key = strxor(ciphers[-1], target_plain)
    print(key)
    print("key: %s" % codecs.encode(key).hex())

# Output
# The secret message is: When using a stream cipher, never use the key more than once

# key: 

# Decrypt all messages using the key:
# [0] We can factor the number 15 with quantum computers. We can also factor the number 15 with a dog trained to bark three times - Robert Harley
# [1] Euler would probably enjoy that now his theorem becomes a corner stone of crypto - Annonymous on Euler's theorem
# [2] The nice thing about Keeyloq is now we cryptographers can drive a lot of fancy cars - Dan Boneh
# [3] The ciphertext produced by a weak encryption algorithm looks as good as ciphertext produced by a strong encryption algorithm - Philip Zimmermann
# [4] You don't want to buy a set of car keys from a guy who specializes in stealing cars - Marc Rotenberg commenting on Clipper
# [5] There are two types of cryptography - that which will keep secrets safe from your little sister, and that which will keep secrets safe from your government - Bruce Schneier
# [6] There are two types of cyptography: one that allows the Government to use brute force to break the code, and one that requires the Government to use brute force to break you
# [7] We can see the point where the chip is unhappy if a wrong bit is sent and consumes more power from the environment - Adi Shamir
# [8] A (private-key)  encryption scheme states 3 algorithms, namely a procedure for generating keys, a procedure for encrypting, and a procedure for decrypting.?
# [9]  The Concise OxfordDictionary (2006) defines crypto as the art of  writing o r solving codes.
# [10] The secret message is: When using a stream cipher, never use the key more than once
